...
> 为什么要用hash扰动函数？
>
> 数组索引值的取法为hash值对数组长度求模，一般数组长度较小，主要是hash值低位参与计算。通过扰动函数可以把高位的特征和低位的特征组合起来，这样参与计算的就是全部特征值，进而降低哈希冲突的概率。
...
JDK 1.8 HashMap 的 hash 方法相比于 JDK 1.7 hash 方法更加简化:
...
- ①如果定位到的数组位置没有元素 就直接插入。
- ②如果定位到的数组位置有元素，遍历以这个元素为头结点的链表，依次和插入的key比较，如果hash和key相同就直接覆盖，不同就采用头插法插入元素。
...
在put键值对的时候判断是否进行扩容。put方法开始时，如果数组为空则进行初始化扩容；put方法末尾，判断元素个数是否超过阈值，超过就进行扩容。



**如何扩容？**

如果数组为空，则生成一个默认大小为16的Node数组。

如果数组不为空，则生成一个大小为旧数组大小两倍的新数组；再将就数组元素逐个添加到新数组中，添加的方法为：遍历数组，节点数据存在
        ...
        for (int j = 0; j < oldCap; ++j) {
            ...
            if ((e = oldTab[j]) != null) {
                ...
                if (e.next == null)
                    ...
                    // 这段代码表示生成两个链表：低索引区域链表和高索引区域链表
                  	// 低索引区域链表放在当前索引j位置
                  	// 高索引区域链表放在j + oldCap索引位置
                  	// 高、低索引通过 (e.hash & oldCap)是否为零来划分
                  	// 划分高低索引，可以分散数据，便于检索、减少碰撞。
